...
- OCSF normalization rules + field mapping expectations → `050_normalization_ocsf.md`,
...
- Telemetry collection invariants (OTel, raw Windows event collection) → `040_telemetry_pipeline.md`
  (details/fixtures may live in `042_osquery_integration.md`)
- Criteria semantics + cleanup verification model → `035_validation_criteria.md`
- Sigma detection model + bridge backend requirements → `060_detection_sigma.md` +
...
- You MAY update other docs (including README) when required to keep the spec set internally
  consistent.
