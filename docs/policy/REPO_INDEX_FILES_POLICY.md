...
- **Index file**: a navigation-only document whose job is to point an agent to authoritative
  sources (entrypoints) with minimal reading.
...
2. **Instruction size cap**: Codex stops adding discovered instruction files once the combined
   size reaches `project_doc_max_bytes` (32 KiB by default). Guidance should be split across
   nested directories rather than bloating a single root file. 
3. **Context is finite and may be compacted**: Codex monitors remaining context and may compact
   (summarize/discard) content over long threads. Navigation docs must therefore be concise and
   high-signal. 

## Policy

### 1) Index hierarchy (MUST)
...
2. **Scoped index** (bounded router + local conventions)
...
3. **Per-pack/per-module index** (inventory allowed)
...
### 2) Naming to reduce ambiguity (SHOULD)
...
### 5) Indexes are navigation-only (MUST)
...
### 6) Maintenance and verification (MUST)
...
2. **Entrypoint completeness check**
...
and instruct agents in `AGENTS.md` when to use it. :contentReference[oaicite:10]{index=10}
...
1. **Prompt injection awareness (MUST)**
   - When enabling network access or web search in Codex, operators MUST treat fetched content as
     untrusted, because prompt injection can cause an agent to follow malicious instructions. :contentReference[oaicite:11]{index=11}

2. **Approval/sandbox posture (SHOULD)**
   - Prefer sandboxed, approval-gated modes for editing/running commands, especially in repos that
     are not version-controlled. :contentReference[oaicite:12]{index=12}

3. **Boundaries and “never touch” lists (SHOULD)**
   - Agent instruction files are most effective when they declare explicit boundaries and required
     commands/tests early. :contentReference[oaicite:13]{index=13}
