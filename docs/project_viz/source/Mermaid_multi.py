...
# Default to the split model directory (architecture/*).
MODEL_PATH_DEFAULT = PROJECT_VIZ_DIR / "architecture"
...
def _sanitize_yaml_for_pyyaml(raw: str) -> str:
    ...
    for line in raw.splitlines():
        match = re.match(r"^(\s*(?:excerpt|definition|description):\s*)(.+)$", line)
        if not match:
...
def _load_yaml_mapping(path: Path) -> dict[str, Any]:
    """Load a YAML file that must parse to a top-level mapping."""
