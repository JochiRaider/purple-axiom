...
- Run an OpenTelemetry Collector (or a distro such as Splunk OTel Collector or Bindplane Agent) on
  each endpoint.
...
- Collection SHOULD be performed via the OTel Collector `filelog` receiver with a JSON parsing
  operator.
...
## Windows Event Log collection (OTel Collector)
...
- A collector MAY attempt to attach rendered message strings for operator convenience, but these
  strings MUST be treated as non-authoritative, MUST be nullable, and MUST NOT participate in
  `metadata.event_id` generation or any stable hashing.
...
- If raw XML is not available for a Windows Event Log record, telemetry validation for that asset
  MUST fail closed (see [Practical validation harness](#practical-validation-harness-required)).
...
a string (the original Windows Event XML when `raw: true`) or a structured map (when not in raw
mode, or when the receiver populates parsed fields). Purple Axiom treats the LogRecord as the
transport envelope and does not assume a single fixed body shape. Normalization converts the
LogRecord plus raw payload into OCSF.

### Large and binary event data

Windows Event Data may contain large strings (example: PowerShell script blocks), nested XML, and
binary blobs (often represented as hex or base64 in XML).
...
Purple Axiom does not prescribe a single set of correct values for all labs. However, to support MVP
deployments and to make bounded queue behavior mechanically testable, Purple Axiom does provide
baseline starter values and a deterministic tuning methodology.
...
collecting Windows Event Log and Sysmon and exporting off-host.
...
- Memory limiter:
  ...
  - `check_interval: 1s`
- Batch processor:
  - `send_batch_size: 1024` (approximately 1000 events)
  - `timeout: 10s`
- Exporter sending queue and retry (per exporter):
...
1. **Size the sending queue to a bounded outage budget**.
   - Decide the maximum export stall you want to tolerate without drops (example: 30 to 120
     seconds).
   - Choose `queue_size approx target_eps * tolerated_stall_seconds`.
...
1. **Tune batching to your EPS**.
   - Pick `send_batch_size` so that a batch represents roughly 0.5 to 2.0 seconds of traffic at
     target EPS.
...
1. **Validate under backpressure and restarts**.
   - Re-run the validation harness backpressure and restart tests (see
     [Practical validation harness](#practical-validation-harness-required)) after each tuning
     change.
...
baseline targets in the [operability spec](110_operability.md) section "Resource budgets" are
intended to provide a reasonable default for typical lab roles (Windows endpoint, DC, Linux server)
and common telemetry profiles.

Sizing guidance:

- **Queue sizing (bounded outage budget)**:
  `queue_size approx sustained_eps_target * tolerated_stall_seconds`.
- Example: for 50 sustained EPS and a 60-second outage budget, choose `queue_size approx 3000`
  (round up).
- **Batch sizing (bounded latency)**: pick `send_batch_size` such that a batch represents roughly
  0.5 to 2.0 seconds of traffic at sustained EPS.
- Example: for 300 sustained EPS, `send_batch_size` in the 150 to 600 range is a reasonable starting
  point.
...
- **Drops and backpressure indicators**:
...
1. Durable checkpoint persistence (collector).
   - For sources that support stable cursors (example: Windows Event Log), the collector
     configuration MUST enable durable state storage for checkpoints.
   ...
   - If the collector is deployed in a container, the checkpoint directory MUST be mounted to a
     persistent volume.
   - For file-tailed sources (example: osquery results via `filelog`), the receiver MUST enable
     durable offset tracking via a storage-backed checkpoint; file offsets MUST NOT be in-memory
     only.
   ...
   - If the chosen storage backend performs automatic corruption recovery by starting a fresh
     database, this MUST be treated as checkpoint loss even if the process continues without
     operator action.
1. Loss and replay handling (pipeline).
...
1. Restart policy (operator-visible).
   ...
   - A restart MUST NOT require manual cleanup of raw stores or normalized stores to recover
     correctness.
1. File-tailed source caveats (osquery NDJSON and similar).
   - osquery filesystem logger rotation may produce compressed rotated files; operators MUST ensure
     that the chosen rotation and retention scheme preserves a sufficient window of readable NDJSON
     for the collector to catch up after a restart or outage.
   - **osquery `.zst` rotation caveat (required)**: If osquery filesystem logger rotation is
     enabled, older rotated segments may be Zstandard-compressed (`*.zst`). The OTel `filelog`
     receiver MUST NOT be assumed to read `*.zst` segments; v0.1 only assumes plain-text NDJSON and
     optional gzip (`compression: gzip`). Operators MUST either tune rotation and retention so the
     collector catch-up window never requires reading `*.zst` (example: only the active file and
     `.1` exist during the run window plus outage budget), or implement an explicit
     decompress-to-plain (or gzip) staging step prior to `filelog` tailing. If `*.zst` segments are
     the only available rotated history, any resulting gaps MUST be recorded as telemetry gaps
     during validation.
   - The `filelog` receiver `include` patterns MUST cover both the active file and any rotated
     segments that remain readable during the worst-case catch-up window (outage budget plus restart
     time). If rotated segments age out of the include glob before being read, loss is expected and
     MUST be treated as a telemetry gap.
   - If rotated segments are gzip-compressed, the collector MUST be configured with
     `compression: gzip` for those files and the compressed file MUST be append-only
     (recompress-overwrite patterns are not supported for correctness).
   - Telemetry validation MUST include a crash/restart and rotation continuity test for every
     enabled file-tailed source (see
     [Practical validation harness](#practical-validation-harness-required)).
...
### Processing-time enrichment (recommended)

Collectors run continuously; the Purple Axiom pipeline assigns `run_id` and `scenario_id` during
processing and normalization using manifest start and end bounds, scenario ground-truth timestamps,
and asset identity. This avoids per-run restarts or reconfiguration of collectors and is stable
under at-least-once delivery.

### Collection-time enrichment (optional)
...
Throttle the exporter or disk and confirm bounded queue behavior (drops are explicit and counted).

### Footprint

Measure CPU and RAM at idle and at target EPS (events per second) for 10 minutes.
...
## Payload limits and binary handling (Windows Event Log)

### Definitions

- **Raw event XML**: the canonical event payload captured in raw or unrendered mode.
- **Binary event data**: event payload elements that represent binary blobs (for example `<Binary>`
  or BinaryEventData in the event schema) and large encoded payload fields embedded in XML. This
...
1. Binary extraction rules (optional but mechanically testable).
   - If the raw XML contains a binary-like field value (hex or base64) and its decoded length is \<=
     ...
     and record `binary_ref` and `binary_sha256` (SHA-256 of decoded bytes) in the raw Parquet row.
   - If the decoded length would exceed `max_binary_bytes`, the pipeline MUST NOT write decoded
     bytes to sidecar and MUST record a deterministic summary instead (at minimum
...
1. Sidecar blob store (when enabled).
   ...
   - Sidecar writes MUST respect redaction posture; when `security.redaction.enabled=false`, sidecar
     payloads MUST follow the same withhold or quarantine rules as other evidence-tier artifacts
     (see the [security and safety spec](090_security_safety.md)).

### Raw or unrendered Windows Event Log failure modes

This subsection defines deterministic behavior when the Windows Event Log receiver cannot provide
raw or unrendered XML, or when rendering metadata is unavailable due to missing provider manifests
or publisher metadata failures.

#### Receiver capability assumptions (OTel windowseventlog)

The OTel `windowseventlog` receiver supports `raw: true` (body is the original XML string) and
`include_log_record_original: true` (adds `log.record.original` attribute containing the original
XML). Operators MAY mutate `body`, so `log.record.original` is the preferred escape hatch when
present. The receiver also supports `suppress_rendering_info: true` to avoid extra rendering
syscalls; this may produce unresolved values but must not prevent collection of raw XML.
...
If publisher or manifest metadata is missing such that event rendering fails (for example, missing
...
- Treat rendering metadata unavailability as non-fatal for the raw or unrendered invariant.
...
- [Configuration reference](120_config_reference.md)
- [Security and safety spec](090_security_safety.md)
