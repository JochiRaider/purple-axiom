...
## Summary
A local-first cyber range that continuously runs safe adversary-emulation scenarios, collects telemetry, normalizes events into OCSF, evaluates detections (Sigma), and produces reproducible scorecards.

## Motivation
- Replace ad-hoc “run a test, eyeball logs” workflows with repeatable ground-truth runs and measurable detection outcomes.
...
- Re-run produces identical outputs given same inputs (or clearly documented sources of nondeterminism).
