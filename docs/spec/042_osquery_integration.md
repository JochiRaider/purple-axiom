...
- Each line MUST include exactly one of `columns` (object) or `snapshot` (array of objects).
...
- The normalizer MUST interpret `unixTime` as integer epoch seconds and derive `time` as epoch
...
| `action`          | string        | `added`, `removed`, or `snapshot`             |
| `columns_json`    | string (JSON) | Present for `added` or `removed`              |
...
- `raw_json` MUST be the original parsed JSON object re-serialized via RFC 8785 (JCS) prior to
  hashing or stable joins (see ADR-0002 "Event Identity and Provenance").
...
`metadata.event_id` MUST be computed per ADR-0002 "Event Identity and Provenance" using an osquery
identity basis with deterministic inputs.
...
- `payload` MUST be the canonical JSON object or array for `columns` or `snapshot` using RFC 8785
  (JCS).
- `calendarTime` MUST NOT be included.
- For OCSF-conformant outputs, `metadata.uid` MUST equal `metadata.event_id` (see the
...
explanation in `unmapped.osquery.notes`.
...
recorded in `unmapped.osquery.notes`.
...
- **Parsing**: Valid JSON lines are ingested and available to normalization. Invalid JSON lines are
  counted as ingest or parse errors and do not produce normalized events.
- **Routing**: Known `query_name` values map to the expected `class_uid`. Unknown `query_name`
  values do not produce normalized events and are recorded as unrouted or unmapped.
- **Identity determinism**: Re-normalizing the same fixture input produces byte-identical
  `metadata.event_id` values (and `metadata.uid` when present).
- **Raw preservation**: The staged `runs/<run_id>/raw/osquery/osqueryd.results.log` is present when
  osquery is enabled. Routed normalized events contain `unmapped.osquery.raw_json` (or an explicitly
  redacted-safe representation when redaction is enabled).
