...
Purple Axiom is a **local-first** cyber range for **isolated lab environments** that runs safe adversary-emulation scenarios, captures telemetry from lab assets, normalizes events into **OCSF**, evaluates detections using **Sigma**, and produces **reproducible run bundles** (artifact bundles suitable for regression testing and trend tracking).  
(Sources: `docs/spec/000_charter.md` “Charter”; `docs/spec/010_scope.md` “Scope”; `docs/spec/020_architecture.md` “Overview”; `docs/spec/080_reporting.md` “Reporting goals”; existing `README.md` “Why Purple Axiom”)

## Why Purple Axiom

Most detection engineering loops still look like: run a test, eyeball logs, and call it “good enough.” Purple Axiom specifies a repeatable, defensible loop:
...
(Sources: `docs/spec/020_architecture.md` “Pipeline stages”; `docs/spec/030_scenarios.md` “Ground truth”; `docs/spec/040_telemetry_pipeline.md` “Telemetry pipeline”; `docs/spec/050_normalization_ocsf.md` “Normalization”; `docs/spec/060_detection_sigma.md` “Detection model”; `docs/spec/070_scoring_metrics.md` “Metrics”; `docs/spec/080_reporting.md` “Run bundle reporting”; existing `README.md` “Why Purple Axiom”)
...
Purple Axiom is designed for **isolated lab environments** and emphasizes detectability validation rather than stealth, persistence, or destructive outcomes.  
(Sources: `docs/spec/010_scope.md` “Scope and non-goals”; `docs/spec/090_security_safety.md` “Safety model”; existing `README.md` “Scope and safety”)
...
- Long-term artifacts MUST avoid storing secrets; redaction is configurable and can be tuned or disabled (with explicit handling for unredacted outputs).  
- The detection subsystem MUST treat Sigma as non-executable content (no arbitrary code execution).  
(Sources: `docs/spec/090_security_safety.md` “Safety constraints” + “Rule execution model”; `docs/spec/035_validation_criteria.md` “Cleanup verification”; `ADR-0003-redaction-policy.md`; existing `README.md` “Hard safety constraints”)
...
2. **Telemetry pipeline** captures raw events (via OpenTelemetry Collector topology) and preserves original semantics.
3. **Normalizer** converts raw events to OCSF envelopes with required provenance and deterministic identity.
4. **Validation** applies criteria packs and cleanup verification.
5. **Detection** evaluates Sigma rules against normalized events (via the Sigma-to-OCSF bridge).
6. **Scoring** produces coverage/latency/gap metrics.
7. **Reporting** emits an HTML + JSON report bundle for diffing and trending.  
(Sources: `docs/spec/020_architecture.md` “Stages”; `docs/spec/040_telemetry_pipeline.md` “Canonical topology”; `docs/spec/050_normalization_ocsf.md` “Normalization pipeline”; `docs/spec/035_validation_criteria.md` “Criteria evaluation”; `docs/spec/065_sigma_to_ocsf_bridge.md` “Bridge”; `docs/spec/070_scoring_metrics.md`; `docs/spec/080_reporting.md`)
...
* **Ground truth**: append-only timeline of what executed, with hashed summaries and “expected telemetry” hints.
* **Normalized events**: OCSF event envelopes with required provenance fields and deterministic `metadata.event_id`.
* **Criteria pack**: validation rules that evaluate expected vs observed telemetry and cleanup state.
* **Sigma-to-OCSF bridge**: mapping/router layer that makes Sigma evaluation over OCSF mechanically testable.
* **Run bundle**: the on-disk artifact bundle rooted at `runs/<run_id>/`, indexed by `manifest.json`.
  (Sources: `docs/spec/030_scenarios.md` “Scenario model”; `docs/spec/025_data_contracts.md` “Run bundle”; `docs/spec/050_normalization_ocsf.md` “OCSF envelopes”; `ADR-0002-event-identity-and-provenance.md`; `docs/spec/035_validation_criteria.md`; `docs/spec/065_sigma_to_ocsf_bridge.md`)
...
(Sources: `docs/spec/025_data_contracts.md` “Run bundle layout” + “Manifest”; `docs/spec/080_reporting.md` “Run bundle reporting goals”)
...
* Normalized event storage MAY be JSONL or Parquet depending on the contract and use case; Parquet is the default for long-term analytics storage, while JSONL is used where line-addressable diffs and fixtures are required.
* Outputs that are intended to be diffed (JSON/JSONL) MUST use deterministic ordering rules as specified in the storage/contracts documentation.
  (Sources: `docs/spec/045_storage_formats.md` “Storage formats”; `docs/spec/025_data_contracts.md` “Deterministic writing”; `docs/spec/100_test_strategy_ci.md` “Golden run fixtures”)

## Determinism and provenance (what makes outputs diffable)

* `run_id` is immutable per execution and is carried across artifacts via required provenance fields.
* `metadata.event_id` is deterministic when the source event and normalization inputs match (see event identity ADR).
...
* Mapping profiles and bridge inputs are snapshot to make evaluations reproducible across time.
  (Sources: `ADR-0002-event-identity-and-provenance.md`; `docs/spec/025_data_contracts.md` “Canonical JSON” + “Cross-artifact invariants”; `docs/spec/050_normalization_ocsf.md` “Provenance requirements”; `docs/spec/065_sigma_to_ocsf_bridge.md` “Snapshotting”)

## Configuration

Purple Axiom is configured via a `range.yaml` (see the normative configuration reference).
(Sources: `docs/spec/120_config_reference.md` “Configuration reference”; existing `README.md` “Configuration”)

* Canonical keys, defaults, and examples: `docs/spec/120_config_reference.md`
* Operational limits and safety toggles (including redaction and signing): `docs/spec/120_config_reference.md` + `docs/spec/090_security_safety.md` + `ADR-0003-redaction-policy.md`
...
* Cross-artifact invariant checks (e.g., `run_id` consistency, referential integrity across event IDs)
* Golden run end-to-end fixtures for regression testing
* Report generation sanity checks
  (Sources: `docs/spec/100_test_strategy_ci.md` “CI strategy”; `docs/spec/025_data_contracts.md` “Contracts and invariants”; `docs/spec/080_reporting.md` “Report outputs”; existing `README.md` “CI and validation”)
...
* `ADR-0003-redaction-policy.md`
  (Sources: existing `README.md` “Documentation (specs)”; ADR and spec file headers)
...
* Isolated lab environment (required)
  (Source: existing `README.md` “Requirements”)

## License

MIT License. See `LICENSE`.
(Source: existing `README.md` “License”)
...
* clear provenance and reproducibility metadata
  (Source: existing `README.md` “Contributing”; reinforced by `docs/spec/025_data_contracts.md` + `docs/spec/090_security_safety.md`)
