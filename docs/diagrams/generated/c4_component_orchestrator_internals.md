...
Rel(lab_provider, lab_provider_sources, "import inventory (resolver-specific)", "filesystem_or_api")
Rel(lab_provider, contract_registry, "validate inventory snapshot against contracts", "filesystem")
Rel(lab_provider, runner, "provide inventory snapshot via run bundle logs/", "filesystem")
Rel(runner, atomic_red_team, "execute Atomic Red Team tests (v0.1)", "framework_usage")
Rel(runner, invoke_atomic_red_team, "invoke executor module in remote session", "PowerShell Remoting")
Rel(runner, telemetry, "provide ground truth timestamps for telemetry collection window", "filesystem")
Rel(normalization, mapping_profile_pack, "load mapping profiles", "filesystem")
Rel(normalization, validation, "provide normalized events", "filesystem")
Rel(validation, scoring, "provide criteria results", "filesystem")
Rel(detection, sigma_rule_packs, "load Sigma rule packs", "filesystem")
Rel(detection, sigma_to_ocsf_mapping_packs, "resolve Sigma-to-OCSF mapping pack", "filesystem")
Rel(detection, sigma_to_ocsf_bridge_adapter, "compile/evaluate Sigma via bridge adapter", "in_process")
Rel(detection, scoring, "provide detections", "filesystem")
Rel(scoring, reporting, "provide scoring outputs", "filesystem")
