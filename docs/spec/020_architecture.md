...
[Lab Provider] -> [Resolved Lab Inventory Snapshot] | v [Scenario Runner] -> \[Ground Truth Timeline
\+ Runner Artifacts\] | v [Telemetry Collectors] | v [OTel Collector Pipeline] | v \[Normalizer ->
OCSF Event Store\] | v [Criteria Evaluator (sidecar pack) -> Criteria Results] | v\
[Sigma-to-OCSF Bridge (router + field aliases)] | v [Detection Engine (Sigma) -> Results] | v
...
- Produces a run-scoped “lab inventory snapshot” recorded in the run bundle and referenced by the
  manifest.
...
- MUST record resolved target identifiers (asset_id plus resolved host identity) so that ground
  truth remains valid even if the provider inventory changes later.
...
- Endpoint logs (Windows Event Log receiver for OTel contrib; includes Sysmon Operational channel on
  Windows; Linux journald/syslog options later).
- Optional: osquery, EDR exports. Network sensors (pcap/netflow) are phase 2 for collection, but
  v0.1 reserves placeholder contracts.
...
- For each executed action (ground truth), match the appropriate criteria entry and evaluate
  expected signals within configured time windows.
- Emit `criteria_results.jsonl` for scoring and reporting.
- Emit cleanup verification results (either as part of criteria results or as runner-side evidence
  referenced by them).

### Sigma-to-OCSF Bridge

- Contract-driven compatibility layer that allows Sigma rules (logsource + fields) to be evaluated
  against OCSF-normalized events.
- Artifacts:
  - Logsource router: `sigma.logsource` -> OCSF class filter (and optional producer/source
    predicates)
...
- Fail-closed behavior: if the bridge cannot route a rule (unknown `logsource`) or map a referenced
  field, the rule is reported as non-executable for that run.
