...
This charter is intentionally **orienting**. When implementing, prefer the specs/ADRs below as the
authoritative sources for detailed contracts and mechanics.

### Where to look first

- **Run bundle layout, artifact schemas, hashing, join invariants:**
  [Data contracts specification](025_data_contracts.md)
- **Stage ordering, IO boundaries, publish semantics, completion rules:**
  [Deployment architecture ADR (ADR-0004)](../adr/ADR-0004-deployment-architecture-and-inter-component-communication.md)
- **Stage outcomes, reason codes, run status derivation:**
  [ADR-0005](../adr/ADR-0005-stage-outcomes-and-failure-classification.md)
- **Publish gates, registry resolution, deterministic serialization, reader semantics, CI
  conformance:** [Contract Spine specification](026_contract_spine.md)
- **Export/signing scope and `logs/` classification:**
  [ADR-0009](../adr/ADR-0009-run-export-policy-and-log-classification.md)
- **Safety posture, isolation, redaction/quarantine rules:**
  [Security and safety specification](090_security_safety.md) and
  [ADR-0003](../adr/ADR-0003-redaction-policy.md)
...
- Run bundle roots and indices:
  ...
  - `manifest.status`
  - `run_results.json`
- Publish + validation seam:
  - `contract_registry.json`
  - `bindings[].stage_owner`
...
- **Run bundle root**
  ...
  - Path: `runs/<run_id>/`
  - Requirement: the run directory name MUST equal the run id recorded in `manifest.json`.

- **Run lock (outside the bundle)**

  - Path: `runs/.locks/<run_id>.lock`
  - Requirement: the orchestrator acquires `runs/.locks/<run_id>.lock` before creating or mutating
    `runs/<run_id>/`.

- **Workspace root layout (v0.1+; reserved paths)**

  - Anchor: the workspace root (directory containing `runs/`) has reserved top-level directories for
    forward compatibility.
  - Requirements:
    - v0.1 tooling MUST ignore unknown workspace files/directories.
    - v0.1 tooling MUST NOT write persistent artifacts outside the run bundle except under
      explicitly reserved workspace locations (for example `runs/.locks/`, `cache/`, and
      `exports/`).
  - Reference: See the Architecture spec "Workspace layout (v0.1+ normative)".

- **Stage staging root**

  - Path: `runs/<run_id>/.staging/<stage_id>/`
  - Requirements:
    - stages write to `runs/<run_id>/.staging/<stage_id>/`
    - stages MUST publish by per-path atomic replace (rename/replace) into final run-bundle paths
      after publish-gate validation.

- **Stage identifiers vs output directories**

  - Requirements:
    - stage identifiers appear in stage outcomes (`manifest.json` and `logs/health.json` when
      enabled) and do not necessarily match output directory names
    - example: stage `reporting` publishes under `report/`, and stage `signing` publishes under
      `security/`.
...
- **Stage ↔ contract-backed outputs (machine-readable)**
  ...
  - Requirements:
    - `contract_registry.json.bindings[].stage_owner` declares ownership for each contract-backed
      `artifact_glob`
    - it MUST be used to construct publish-gate `expected_outputs[]` without hardcoding
      stage→contract mappings.

- **Contract validation reports**

  - Path: `runs/<run_id>/logs/contract_validation/<stage_id>.json`
  - Requirement: captures deterministic publish-gate contract validation failures (Tier 0 evidence;
    export/signing eligible).

- **Run results summary (recommended)**

  - Path: `runs/<run_id>/run_results.json`
  - Requirement: the compact, contract-backed status/metrics summary used for CI time-to-signal when
    present.

- **Run log**

  - Path: `runs/<run_id>/logs/run.log`
  - Requirement: required human-readable operator log.
  - Notes:
    - It is volatile diagnostics.
    - It is excluded from default exports and signing/checksum scope (ADR-0009).

### Publish semantics and completion (v0.1)
...
Run-level status is recorded in `manifest.json` as `success | partial | failed`.

In v0.1:

- `partial` means artifacts are mechanically usable but one or more quality gates and/or
  warn-and-skip failures occurred.
- `failed` means the run is not mechanically usable (including fail-closed stage failures or missing
  required artifacts).
...
#### Run index and CI decision surface
...
#### Detection, scoring, and reporting
...
#### Regression (optional) and baseline references
