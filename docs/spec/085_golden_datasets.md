...
- For `contracts_version` `0.1.x` (legacy compatibility):
  1. If `runs/<run_id>/normalized/ocsf_events/` exists and contains one or more `*.parquet` files,
     the builder MUST treat it as the preferred source representation.
...
- `includes[]`: array of strings
  ...
  - Implementations MUST validate `includes[]` entries by invoking the canonical glob parser module
    `pa.glob.v1` (glob_v1 semantics; shared with contract registry `bindings[].artifact_glob`
    validation) and MUST NOT accept a divergent glob dialect here.
...
- `excludes[]`: array of strings (optional)
  ...
  - Each entry MUST be a valid `glob_v1` pattern over dataset-release-root relative candidate paths
    (fail closed on invalid patterns).
  - Implementations MUST validate `excludes[]` entries by invoking the canonical glob parser module
    `pa.glob.v1` (glob_v1 semantics; shared with contract registry `bindings[].artifact_glob`
    validation) and MUST NOT accept a divergent glob dialect here.
...
- takes a fixed set of small run bundles as input (stored as test fixtures), including at minimum:
  - one v0.2+ Parquet-only normalized-store run (`contracts_version >= 0.2.0`; has
    ...
    `normalized/ocsf_events.jsonl`) to exercise byte-for-byte copy (marker-assisted) and
    deterministic rewrite (marker-blind),
