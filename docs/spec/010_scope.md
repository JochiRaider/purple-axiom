...
- Telemetry collection from lab assets (endpoint sources required; network sensors are not required
  for v0.1, but placeholder contracts are reserved now).
- Placeholder data contracts for network telemetry (pcap/netflow), without requiring
  capture/ingestion in v0.1.
