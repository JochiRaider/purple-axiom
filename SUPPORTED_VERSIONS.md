...
| GHOSTS (noise engine, server)                                  |          8.3.1 | Runner (benign noise generation)                   | Only required when `runner.environment_config.noise_profile` uses this engine. Pins MUST be recorded in-run (see architecture + data contracts). Version drift triggers version-conformance failure for runs that enabled the engine. Pin container image digest when deployed via container. |
| GHOSTS (endpoint agent)                                        |          8.3.1 | Runner (benign noise generation)                   | Only required when `runner.environment_config.noise_profile` uses this engine. Pins MUST be recorded in-run (see architecture + data contracts). Version drift triggers version-conformance failure for runs that enabled the engine. Pin installer/binary hash when applicable.              |
...
| asciinema-player (bundled UI asset)                            |         3.14.0 | Operator Interface (cast playback; v0.2+)          | Only required when the Operator Interface serves inline `.cast` playback. Assets MUST be bundled locally (no remote fetches) and pinned by version (and integrity hash if packaged). Version drift triggers version-conformance failure for runs that enabled the UI.                         |
...
1. CI MUST fail closed if any dependency version differs from the pins above for an enabled stage.
   This includes optional dependencies when enabled (for example: benign noise engines and bundled
   UI playback assets).
